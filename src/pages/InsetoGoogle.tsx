...
/* ──────────────────────────────────────────────────────────────
   LP GOOGLE ADS — INSETO · /g/inseto
   Variante para o Google da /curiosidade. Existe para a campanha
   `[Non Brand] Search - Inseto` (20/09/26): quem PESQUISA "inseto cachorro",
   "proteina de inseto para caes" e afins. Mesmo molde das outras /g/ —
   utm_source proprio, cupom GOOGLE10 embutido na URL do checkout, noindex.
   Produto-foco: KIT CACHORRO (Original + Suplemento Integral).

   Reusa o CSS da /curiosidade (prefixo .curiosidade-lp / cur-).

   Kit Cachorro · token KQXZ5J7LWK · R$145,00 (Shopify) -> R$130,50 com
   GOOGLE10 (-10%), aplicado sozinho na URL. FRETE GRÁTIS no Kit.
   O GOOGLE10 fica fora do Dia do Cliente (decisao da Olivia, 15/09/26):
   por isso aqui nao existe o flag isDayOfClienteActive.

   🔴 A palavra "racao" foi tirada do texto: o Google leu cinco das sete LPs
   como racao (Keyword Planner, 01/09/26) e a marca nao vende racao. O texto
   diz "comida" onde a instrucao de uso pedia "racao".
────────────────────────────────────────────────────────────── */

const COUPON = "GOOGLE10";   // 10% off — aprovado pela Olivia 10/08/26
const PRICE = "145,00";      // preço cheio (Shopify)
...
const UTM_FALLBACK = {
  utm_source: "lp-inseto-google",
  utm_medium: "lp",
  utm_campaign: "lp-inseto-kit-caes-google",
};
...
const FAQ = [
  {
    q: "Cachorro pode comer inseto? É seguro?",
  ...
  },
  {
    ...
    a: "O <strong>Original</strong> (larvinhas inteiras, pra usar de petisco ou topper) + o <strong>Suplemento Integral</strong> (pó pra misturar na comida). Um cuida do agrado, o outro reforça a nutrição do dia a dia — numa caixa só, com frete grátis.",
  },
  {
  ...
  },
  {
    ...
    a: "Na maioria das vezes o bloqueio é do tutor 😅. Mistura as larvinhas na comida e deixa ele decidir — a taxa de aceitação surpreende. E tem a <strong>garantia da matilha: 14 dias</strong>.",
  },
  {
...
const InsetoGoogle = () => {
  ...
  return (
    <div className="curiosidade-lp">
      {/* noindex: quase identica a /curiosidade — as duas competiriam pela mesma busca
          organica. Existe so pra separar o trafego do Google Ads (utm_source proprio
          + GOOGLE10 embutido) e nunca entra no sitemap. O AdsBot rastreia a landing
      ...
      <section className="cur-hero">
        <div className="cur-hero-inner">
          ...
          <span className="cur-hero-eyebrow">petisco de inseto para cães · 1 ingrediente · feito no rio</span>
          ...
          <div className="cur-hero-price">
            <span className="cur-price-from">Kit Cachorro de <s>R$ {PRICE}</s> por</span>
            <span className="cur-price-now"><small>R$</small>{PRICE_OFF}</span>
        ...
        <div className="cur-section-inner">
          ...
          <ul className="cur-problemas-list">
            <li className="cur-problema-item">
              <b>Comece como topper</b> — jogue as larvinhas por cima da comida de sempre; o cheiro puxa o interesse.
            </li>
            <li className="cur-problema-item">
      ...
      <section className="cur-oferta">
        <div className="cur-oferta-inner">
          ...
          <div className="cur-oferta-coupon-box">
            <div className="cur-oferta-coupon-label">🏷️ seu desconto</div>
            <div className="cur-oferta-coupon-code">10% OFF</div>
            <div className="cur-oferta-coupon-desc">
              Kit Cachorro de <s>R$ {PRICE}</s> por <strong>R$ {PRICE_OFF}</strong> · desconto já aplicado, sem digitar cupom · frete grátis
            </div>
          </div>
      ...
      <div className="cur-sticky-cta">
        <div className="cur-sticky-info">
          ...
          <span className="cur-sticky-price">
            R$ {PRICE_OFF} · 🚚 frete grátis
          </span>
        </div>
        <a href={ctaUrl("sticky")} data-cta="sticky">Comprar →</a>
      </div>

      <LeadPopup slug="inseto-google" />
    </div>
  );
};

export default InsetoGoogle;
